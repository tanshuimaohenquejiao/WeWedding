...
Page({
	...
	bindMoreTap: async function (e) {
		...
		let home = this.data.dataList.list[idx].PRODUCT_HOME;
		let homeDesc = (home == 0) ? '上首页' : '取消上首页';
	...
	_setHome: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/product_home', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'PRODUCT_HOME', home);
				this.setData({
					dataList: this.data.dataList
				});
			});
	...
	_getSearchMenu: function () {
		...
		let sortItem2 = [
			...
			{ label: '价格从高到底', type: 'sort', value: 'price_desc' },
			{ label: '价格从低到高', type: 'sort', value: 'price_asc' },
		];
