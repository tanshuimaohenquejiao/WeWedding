...
Page({
	...
	_setHome: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/news_home', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'NEWS_HOME', home);
				this.setData({
					dataList: this.data.dataList
				});
			});
	...
	bindMoreTap: async function (e) {
		...
		let home = this.data.dataList.list[idx].NEWS_HOME;
